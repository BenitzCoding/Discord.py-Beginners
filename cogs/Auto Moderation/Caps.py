...
class MassCaps(commands.Cog):
    ...
    def Init(self, message):
        ...
        if not self.bot.cache[message.guild.id]["AutoMod"][3]["Ratelimit"]:
            return False

        if self.bot.cache[message.guild.id]["AutoMod"][3]["Ignored"] and str(message.channel.id) in self.bot.cache[message.guild.id]["AutoMod"][3]["Ignored"]:
            return False
        ...
        if not message.author.id in self.store[message.guild.id]:
            self.store[message.guild.id][message.author.id] = {"String": None, "Count": 0, "Percentage": 0}
